...
# Step 3: Calculate Bias, Monte Carlo Standard Deviation (MCSD), 
...
results_partA <- data.frame(
  ...
  Esimated = signif(c(mean(sample_means),mean(sample_medians),
                      mean(sample_trimmed_means)),3),
...
for (i in 1:n_sim) {
  ...
  samples_contaminated[, i] <- c(rexp(n1, rate = lambda1), 
                                 rexp(n2, rate = lambda2))
...
sample_trimmed_means_cont <- apply(samples_contaminated, 2, 
...
results_partB <- data.frame(
  Estimator = c("Mean", "Median", "Trimmed Mean (10%)"),
  Esimated = signif(c(mean(sample_means_cont), mean(sample_medians_cont), 
                      mean(sample_trimmed_means_cont)), 3),
...
In clean data use, the mean the is the overall best estimator.
...
Since mean is skewed by outlier, the only relevant comparison is between median and trimmed mean. Overall Trimmed mean is slightly preferred with a 1.117837 ratio of efficiency
...
The introduction of outliers in the contaminated data scenario dramatically alters this assessment. The sample mean's estimate jumped from 0.998 to 5.900, while its Monte Carlo Standard Deviation (MCSD) went from 0.0641 to 1.01. This highlights its extreme sensitivity to outliers.

In this contaminated setting, the median and 10% trimmed mean proved to be far more robust. Their estimates shifted only slightly, and their MCSD remained small and stable (0.0764 for the median and 0.0769 for the trimmed mean). This stability in the face of extreme observations is the hallmark of a robust estimator and is clearly supported by the simulation results. Overall in the contaminated data, all the estimator have their values increase.
...
In the contaminated data scenario, the 10% trimmed mean is the most efficient robust estimator. In this setting, the goal is to estimate the central tendency of the primary distribution, not the contaminated mixture. The large MSE values for the median and trimmed mean reflect their large bias relative to the mixture distribution mean of 5.9, but this is by design. When comparing the two robust estimators for their intended purpose, the trimmed mean (MSE = 23.20) outperforms the median (MSE = 25.90). Although the median is marginally more precise (lower MCSD), the trimmed mean's smaller bias makes it the more efficient choice overall. The relative efficiency of the trimmed mean to the median is 25.90 / 23.20≈ 1.118, indicating that trimmed mean is preffered.

### Conclusion and Recommendation

The choice of estimator should be guided by the underlying data characteristics and the analytically goal. Whether outliers is present, whether robustness is needed or not. While the sample mean is optimal for clean, well-behaved data, it is unreliable when outliers are present.
...
#### Task 3 - Maximize log-likelihood with `optim()`
...
cat(
  "beta_hat:\n", paste(capture.output(print(beta_hat)), collapse = "\n"), "\n\n",
  "ll_max:", ll_max, "\n",
  "convergence:", conv, "\n"
)
...
$$
\begin{aligned}
\text{Let } g(x) &= \beta_0 + \beta_1\log x\\
\\
...
```{r}
# Odds ratio interval calculation
cat(
  "95% Odds Ratio Interval:\n",
  "Upper range: ", exp(1.59169 * log(1.10)) - 1, "\n",
  "Middle range: ", exp(2.349 * log(1.10)) - 1, "\n",
  "Lower range: ", exp(3.106091 * log(1.10)) - 1, "\n"
)

```

The estimated odds ratio is **1.25**, indicating that for every 10% increase in GDP per capita, a country is about **25% more likely** to have a higher life expectancy. The **95% odds ratio interval** falls between **1.16 and 1.34**, suggesting a consistently positive association between economic growth and higher life expectancy.

Finding the mid point where the probability of being in higher life expectancy is 50%

$-20.382+ 2.349*\log(x) = 0 \\x = \exp(20.382/2.349)$
...
```{r, warning=FALSE, message=FALSE}
# Odds ratio and GDP reduction calculation
half_prop <- exp(20.382 / 2.349)
red1.10 <- half_prop * (1 - 0.1)
ln_red1.10 <- -20.382 + 2.349 * log(red1.10)

cat(
  "Half proportion (exp(20.382 / 2.349)):", round(half_prop, 3), "\n",
  "Reduced GDP (10% lower):", round(red1.10, 3), "\n",
  "Log-odds at reduced GDP:", round(ln_red1.10, 3), "\n",
  "Less likely (exp(ln_red1.10) - 1):", round(exp(ln_red1.10) - 1, 3), "\n",
  "→ About 21% less likely to have higher life expectancy below GDP ≈ 5279.17\n"
)
```
...
Our logistic regression model confirms a significant positive relationship between the logarithm of GDP per capita and the probability of achieving a high life expectancy. The model is defined by the equation:

$Log-odds(High Life Expectancy) = -20.382 + 2.349 * log(GDP per capita)$

The coefficient for the logarithm of GDP per capita (β1 = 2.349) is statistically significant. The use of a logarithmic scale for GDP is important as it indicates that the impact of economic growth on life expectancy is most pronounced at lower income levels. This means an increase in GDP from \$1,000 to \$2,000 per capita provides the same boost to the odds of a high life expectancy as an increase from \$10,000 to \$20,000. This suggests that while economic growth consistently has a positive effect, its marginal impact on health outcomes diminishes as a country becomes wealthier.
...
-   **Tailor Strategies:** For countries far below this threshold, broad economic development and investment in fundamental infrastructure are critical prerequisites for improving life expectancy. For nations already above the threshold, health investments might focus more on advanced healthcare, lifestyle-related diseases, and promoting well-being in aging populations.
