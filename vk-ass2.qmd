...
```{r}
# Plot the different beta and the log likelihood for each of the data that we have
#beta1_grid
beta0_hat
# plot 1 liklihood of one data
beta_loglik_each = numeric(length)
for (i in 1:length(beta1_grid)){
  beta1 = beta1_grid[i]
  beta_loglik_each[i] = log_lik(c(beta0_hat, beta1), data$log_gdp[1],data$high_lifeExp[1])
}

plot(beta1_grid, beta_loglik_each, 
     type = "l",                      # line plot
     col = "darkred",                # line color
     lwd = 2,                        # line width
     xlab = expression(beta[1]),     # x-axis label with math notation
     ylab = "Likelihood",            # y-axis label
     main = "Likelihood vs Beta1") 
